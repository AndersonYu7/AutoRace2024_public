...
class detect(Node):
    def __init__(self):
        ...
        self.publisher_lane = self.create_publisher(
            Image,
            '/detect/lane',  # 新增的車道線topic
            10)

        self.publisher_control_lane = self.create_publisher(
            Float64,
            '/control/lane',
            10)
        ...
        if self.calibration_mode == True:
            # pub
            self.publisher_birdseye = self.create_publisher(
                Image,
                'birdseye_image_topic',  # 替換為你的鳥瞰圖topic
                10)
            self.publisher_yellow = self.create_publisher(
                Image,
                'yellow_image_topic',  # 替換為你的黃色線topic
                10)
            self.publisher_white = self.create_publisher(
                Image,
                'white_image_topic',  # 替換為你的白色線topic
                10)
            ...
            #sub
            self.subscription_top_x = self.create_subscription(Float64, 'top_x_topic', self.top_x_callback, 10)
            self.subscription_top_y = self.create_subscription(Float64, 'top_y_topic', self.top_y_callback, 10)
            self.subscription_bottom_x = self.create_subscription(Float64, 'bottom_x_topic', self.bottom_x_callback, 10)
            self.subscription_bottom_y = self.create_subscription(Float64, 'bottom_y_topic', self.bottom_y_callback, 10)
            self.subscription_hue_white_l = self.create_subscription(Float64, 'hue_white_l_topic', self.hue_white_l_callback, 10)
            self.subscription_hue_white_h = self.create_subscription(Float64, 'hue_white_h_topic', self.hue_white_h_callback, 10)
            self.subscription_saturation_white_l = self.create_subscription(Float64, 'saturation_white_l_topic', self.saturation_white_l_callback, 10)
            self.subscription_saturation_white_h = self.create_subscription(Float64, 'saturation_white_h_topic', self.saturation_white_h_callback, 10)
            self.subscription_lightness_white_l = self.create_subscription(Float64, 'lightness_white_l_topic', self.lightness_white_l_callback, 10)
            self.subscription_lightness_white_h = self.create_subscription(Float64, 'lightness_white_h_topic', self.lightness_white_h_callback, 10)
            self.subscription_reliability_white_line = self.create_subscription(Float64, 'reliability_white_line_topic', self.reliability_white_line_callback, 10)
            self.subscription_hue_yellow_l = self.create_subscription(Float64, 'hue_yellow_l_topic', self.hue_yellow_l_callback, 10)
            self.subscription_hue_yellow_h = self.create_subscription(Float64, 'hue_yellow_h_topic', self.hue_yellow_h_callback, 10)
            self.subscription_saturation_yellow_l = self.create_subscription(Float64, 'saturation_yellow_l_topic', self.saturation_yellow_l_callback, 10)
            self.subscription_saturation_yellow_h = self.create_subscription(Float64, 'saturation_yellow_h_topic', self.saturation_yellow_h_callback, 10)
            self.subscription_lightness_yellow_l = self.create_subscription(Float64, 'lightness_yellow_l_topic', self.lightness_yellow_l_callback, 10)
            self.subscription_lightness_yellow_h = self.create_subscription(Float64, 'lightness_yellow_h_topic', self.lightness_yellow_h_callback, 10)
    ...
    def image_callback(self, msg):
        ...
        if self.calibration_mode == False:
            ...
            print('self.left_fitx', self.left_fitx)
    ...
    def maskWhiteLane(self, image):
        ...
        if self.calibration_mode == True:
            # self.publisher_white.publish(self.cv_bridge.cv2_to_imgmsg(mask, encoding='bgr8'))
    ...
    def maskYellowLane(self, image):
        ...
        rclpy.logging.get_logger('detect_node').info("YELLOW : %d" % msg_white_yellow_reliability.data)
